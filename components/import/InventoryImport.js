...
const COLUMN_MAP = {
  ...
  'stock qty': 'stockQty', 'min stock': 'minStock', 'unit cost': 'unitCost',
  'supplier': 'supplier', 'expiry date': 'expiryDate', 'notes': 'notes',
}
...
export default function InventoryImport() {
  ...
  return (
    <div className="space-y-3">
      <p className="text-xs text-gray-500">Export the Inventory tab from Google Sheets as CSV</p>
      ...
      {step === 'preview' && (
        <div className="bg-white rounded-xl border border-gray-100 p-4">
          <div className="flex items-center justify-between mb-3">
            <p className="text-xs text-gray-500">{rows.length} items found</p>
          ...
          <table className="w-full text-xs">
            <thead><tr className="border-b border-gray-100">{['Name','Category','Stock Qty','Unit Cost'].map(function(c) { return <th key={c} className="text-left text-gray-400 font-medium pb-1.5 pr-3">{c}</th> })}</tr></thead>
            <tbody>{rows.slice(0,5).map(function(r,i) { return <tr key={i} className="border-b border-gray-50"><td className="py-1.5 pr-3 text-gray-600">{r['Name']||'—'}</td><td className="py-1.5 pr-3 text-gray-600">{r['Category']||'—'}</td><td className="py-1.5 pr-3 text-gray-600">{r['Stock Qty']||'—'}</td><td className="py-1.5 pr-3 text-gray-600">{r['Unit Cost']||'—'}</td></tr> })}</tbody>
          </table>
        </div>
      )}
      {step === 'done' && results && (
        <div className="bg-green-50 border border-green-100 rounded-xl p-4 flex items-center justify-between">
          <p className="text-sm text-green-700">{results.imported} imported{results.failed > 0 && ' · ' + results.failed + ' failed'}</p>
