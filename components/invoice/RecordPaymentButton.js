...
// Push #4 first wave: record a payment against an existing unpaid invoice.
// Used when Dr. Shobhna forgot to enter payment at visit close, or when
// patient pays later. Creates a Receipt linked to the invoice and updates
// the invoice paid/balance/status.
...
export default function RecordPaymentButton({ invoice }) {
  ...
  if (outstanding <= 0) return null  // hidden when no dues
          ...
          <div className="bg-white rounded-xl border border-slate-200 shadow-xl w-full max-w-md">
            <div className="p-5 border-b border-slate-100">
              ...
              <p className="text-sm text-slate-500 mt-1">
                Against invoice <span className="font-medium text-slate-700">{invoice.invoiceNo}</span> · outstanding <span className="font-medium text-red-700">{formatINR(outstanding)}</span>
              </p>
            </div>
              ...
              <div>
                ...
                {Number(amount) > 0 && Number(amount) < outstanding && (
                  <p className="text-xs text-amber-700 mt-1">
                    After this, ₹{Math.round(outstanding - Number(amount)).toLocaleString('en-IN')} will remain outstanding.
                  </p>
                )}
              </div>
